 ...
 * @author Josias Sena
 */
class ProfileUpdaterTest : BaseUnitTest() {
    ...
    fun testGetSessionTokenErrorSdkNotInit() {
        ...
        val profileToUpdate = Mockito.mock(Profile::class.java)
        val testObserver = TestObserver.create<Profile>()

        ProfileUpdater().updatePersonalInfo(token, profileToUpdate).subscribe(testObserver)

        with(testObserver) {
            awaitTerminalEvent()
            assertError(EraSdkUnInitializedException::class.java)
            assertErrorMessage("Error updating personal information. Please make sure the " +
    ...
    fun testErrorBecauseOfExpiredToken() {
        ...
        ProfileUpdater().updatePersonalInfo(expiredToken, profileToUpdate).subscribe(testObserver)

        with(testObserver) {
            awaitTerminalEvent()
            assertError(SessionTokenExpiredException::class.java)
            assertErrorMessage("Cannot update personal information. The session token " +
    ...
    @Test
    fun testUpdateProfileInformationSuccess() {
        SdkInitiatedValidator.isSdkInit = true
    ...
    @Test
    fun testUpdateProfileInformationTimeoutOver14Seconds() {
        SdkInitiatedValidator.isSdkInit = true
